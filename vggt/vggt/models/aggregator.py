...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        ...
        for depth in range(self.aa_block_num):
...
def get_attn_score(blk_class, x, frame_num, token_length, xpos=None):
    ...
    score = []
    for head_i in range(blk_class.attn.num_heads):
        score.append(torch.softmax((q.permute(0, 2, 1, 3) * blk_class.attn.scale @ k.permute(0, 2, 1, 3).transpose(-2, -1))[:, head_i], dim=-1).reshape(B, frame_num, token_length, frame_num, token_length).cpu())
    score = torch.stack(score).mean(dim=0)
    return score

def get_attn_mask(blk_class, x, frame_num, token_length, xpos=None):
    x = blk_class.norm1(x)
    ...
    score = (q.permute(0, 2, 1, 3) * blk_class.attn.scale @ k.permute(0, 2, 1, 3).transpose(-2, -1)).sum(dim=1)
