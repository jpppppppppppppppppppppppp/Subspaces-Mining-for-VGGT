...
def viser_wrapper(
    ...
    if masks is not None:
        masks = masks.squeeze().cpu().numpy().astype(bool)
        colors = colors[masks]
        world_points = world_points[masks]
        conf = conf[masks]
...
parser.add_argument("--use_mask", action="store_true", help="Use mask for segmentation")
...
def main():
    ...
    --D: Path to folder containing input images
    ...
    if args.interval < 0:
        args.interval = 30 if args.image_folder.endswith('.mp4') else 1
    ...
    def seed_anything(seed: int):
        ...
        torch.backends.cudnn.benchmark = False  # Disable non-deterministic algorithms for consistency

        # For other libraries, e.g., TensorFlow, Triton, etc., add similar seed settings if required
    ...
    # model = VGGT()
    # checkpoint = torch.load("./training/logs_THuman_fft_final_123/exp001/ckpts/checkpoint.pt", map_location=device)['model']
    ...
    # model = PeftModel.from_pretrained(model, config=lora_config, model_id="./training/logs_THuman_ours_uniform_16r_123/exp000/ckpts/lora-save")
    # model = PeftModel.from_pretrained(model, config=lora_config, model_id="./training/logs_THuman_lora_16_123/exp001/ckpts/lora-save")
    # model = PeftModel.from_pretrained(model, config=lora_config, model_id="./training/logs_THuman_lora_pissa_16_123/exp001/ckpts/lora-save")
    ...
    if os.path.isdir(args.image_folder):
        print(f"Loading images from directory: {args.image_folder}")
        filenames = sorted([x for x in os.listdir(args.image_folder) if x.lower().endswith(('color.png', '.jpg', '.jpeg'))])
        if args.use_mask:
            mask_filenames = sorted([x for x in os.listdir(args.image_folder) if x.lower().endswith('.exr')])
            masks = []
        for i in range(0, len(filenames), args.interval):
            img_path = os.path.join(args.image_folder, filenames[i])
            try:
                sources.append(Image.open(img_path).convert('RGB'))
                if args.use_mask:
                    depth_path = os.path.join(args.image_folder, mask_filenames[i])
                    depth = read_depth(depth_path, 1.0)
                    mask = depth > 1e-4
                    masks.append(mask)
            except Exception as e:
                print(f"Could not load image {filenames[i]}: {e}")
    elif args.image_folder.lower().endswith('.mp4'):
        print(f"Loading frames from video: {args.image_folder}")
        cap = cv2.VideoCapture(args.image_folder)
        if not cap.isOpened(): raise IOError(f"Cannot open video file: {args.image_folder}")
        frame_idx = 0
        while True:
            ret, frame = cap.read()
            if not ret: break
        ...
        raise ValueError(f"Unsupported path. Must be a directory or a .mp4 file: {args.image_folder}")
    print(f"Found {len(sources)} images")

    images = load_and_preprocess_images(sources).to(device)
    if args.use_mask:
        masks = torch.from_numpy(np.stack(masks)).unsqueeze(1).to(device)
        images = masks * images + (~masks) * 1.0
    print(f"Preprocessed images shape: {images.shape}")

    # from evalutae_on_illusion import CustomDataset
    # dataset = CustomDataset("/home/jiangyu/datasets/illustion")
    # images = dataset[8038]["stereo"].to(device)
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast(dtype=dtype):
            predictions = model(images=images, where=1.0)
    # torch.cuda.memory._dump_snapshot("nsa.pickle")
    ...
    for key in predictions.keys():
        if isinstance(predictions[key], torch.Tensor) and key != "xpos":
    ...
    viser_server = viser_wrapper(
        ...
        masks=masks if args.use_mask else None,
    )
