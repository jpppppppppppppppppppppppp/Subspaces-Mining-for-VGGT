...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        self.is_lora, self.is_adalora = is_lora
        self.is_lora = self.is_lora or self.is_adalora
        ...
        if seed is not None:
            ...
            self.logging_conf.log_dir = self.logging_conf.log_dir + '_' + str(seed)
        else:
        ...
        if self.mode != "val" and not self.is_lora:
        ...
        if self.mode != "val" and self.is_lora:
            self.optims = construct_optimizers(self.model, self.optim_conf)
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if self.is_lora:
            ...
            lora_config.target_modules = lora_modules_name
    ...
    def _setup_components(self):
        ...
        if getattr(self.optim_conf, "frozen_module_names", None) and not self.is_lora:
            logging.info(
    ...
    def save_checkpoint(self, epoch: int, checkpoint_names: Optional[List[str]] = None):
        ...
        if self.is_lora:
            if (
                ...
                model.save_pretrained(os.path.join(checkpoint_folder, f"lora-save-{epoch}"))
            model.save_pretrained(os.path.join(checkpoint_folder, f"lora-save"))
        else:
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            exact_step = self.epoch * (limit_train_batches + 1) + data_iter
            ...
            self.scaler.update()
            if self.is_adalora:
                self.model.module.base_model.update_and_allocate(exact_step)
