...
class THUDataset(BaseDataset):
    def __init__(
        ...
        category = sorted(os.listdir(THU_DIR))

        if self.debug:
            category = [category[0]]

        if split == "train":
            category = category[:int(len(category))-100]
        ...
        logging.info(f"THU_DIR is {THU_DIR}")

        self.THU_DIR = THU_DIR

        total_frame_num = 0

        for seq_name in category:
            anno_file_path = osp.join(THU_DIR, seq_name, "opencv_cameras.json")
            if not osp.exists(anno_file_path):
                print("no anno file", seq_name)
                continue
        ...
        logging.info(f"{status}: THuman Data size: {self.sequence_list_len}")
        logging.info(f"{status}: THuman Data total frames size: {self.total_frame_num}")
        logging.info(f"{status}: THuman Data dataset length: {len(self)}")
    ...
    def get_data(
        ...
        for anno in annos:
            ...
            image_path = osp.join(self.THU_DIR, seq_name, filepath)
            ...
            norm = np.linalg.norm(extri_opencv, axis=0, ord=2)
            fac = np.mean(norm[:3])
            extri_opencv /= fac
            fx = anno['fx']
            fy = anno['fy']
...
class THUDataset_small(BaseDataset):
    def __init__(
        self,
        common_conf,
        split: str = "train",
        ...
        if split == "train":
            category = category[:int(len(category))-100]
            idx = range(0, len(category), 10)
            category = [category[id] for id in idx]
            self.len_train = len_train
        elif split == "test":
            category = category[int(len(category))-100:]
            self.len_train = len_test
