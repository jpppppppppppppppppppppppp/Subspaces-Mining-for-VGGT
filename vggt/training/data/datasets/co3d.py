            ...
            for split_name in split_name_list:
                ...
                for seq_name, seq_data in annotation.items():
                    ...
                    if seq_name in self.invalid_sequence:
                        continue
                    if not os.path.exists(os.path.join(self.CO3D_DIR, c, seq_name)):
                        print("not exists", c, seq_name)
                        continue
    ...
    def get_data(
        ...
        for anno in annos:
            ...
            if self.load_depth:
                ...
                print("first depth map shape:", depth_map.shape, filepath, np.max(depth_map), np.min(depth_map))
                ...
                print("second depth map shape:", depth_map.shape, filepath, np.max(depth_map), np.min(depth_map))

                depth_map = threshold_depth_map(
                    depth_map, min_percentile=-1, max_percentile=98
                )
                
                print("third depth map shape:", depth_map.shape, filepath, np.max(depth_map), np.min(depth_map))
            else:
                depth_map = None
            ...
            print("final depth map shape:", depth_map.shape, filepath, np.max(depth_map), np.min(depth_map), target_image_shape)
            images.append(image)
