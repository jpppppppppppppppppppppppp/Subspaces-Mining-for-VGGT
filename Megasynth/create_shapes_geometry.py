...
class Shape(object):
    ...
    def genObj(self, filePath, out_dir, bMat=False, bComputeNormal=False, bScaleMesh=False, bMaxDimRange=[0.3, 0.5]):
        ...
        material_ids = []
        mat = matsynth()
        mat_paths = mat.get_matsynth_material(os.path.join(out_dir, "..", "materials"), num_samples=20)
        with open(mtl_path, "w") as f:
            for im in range(len(self.matStartId)):
                f.write("newmtl %s \n" % (self.matNames[im]))
                
                # TODO switch to sampling different materials instead of fixed one
                material_id = random.choice(range(len(mat_paths)))
                mat_path = mat_paths[material_id]  # random select a material
...
class matsynth:
    cached_output = []   
    def __init__(self):
        self.base_dir = "/scratch/10807/yujiang_314159/megasynth/geometry_no_variation_2/materials"
        self.ds = sorted(os.listdir(self.base_dir))

    def get_matsynth_material(self, base_output_dir, num_samples=1, group_id=42):
        ...
        # shuffle data
        # self.ds = self.ds.shuffle(buffer_size=1)
        ...
        # save_dirs = ["/scratch/10807/yujiang_314159/megasynth/scenesacg_tiles_023"]
        if len(matsynth.cached_output) == 0:
            ...
            with temp_seed(group_id):
                idx = np.random.randint(0, len(self.ds), num_samples)
                for i in idx:
                    save_dirs.append(os.path.join(self.base_dir, self.ds[i]))

            matsynth.cached_output = save_dirs
        # for i, x in enumerate(self.ds['train']):
        #     if i == num_samples:
        #         break
        #     save_dir = Path(os.path.join(base_output_dir, x['name']))
        #     save_dirs.append(str(save_dir.resolve()))
...
if __name__ == "__main__":
    ...
    mat_path = matsynth().get_matsynth_material(out_dir)
