...
from common import *
from create_shapes_geometry import *
...
CUBE_TO_SHAPE = {
    'lB': {"sub_obj_nums": [4, 5, 6, 7, 8, 9], "sub_obj_num_poss": [5, 7, 10, 10, 7, 5]},   
...
class Scene(Shape):
    def __init__(self, 
                # scene scale info
                scene_size_range=(12.0, 15.0), 
                scene_height_range=(12.0, 18.0), 
                ...
                max_num_large_cubes=6, 
                max_num_small_cubes=6,
                max_num_roof_cubes=0, 
                max_num_wall_cubes=0,
                max_num_frame_cubes=0,
                max_num_wall_thin_cubes=0,
                max_num_space_thin_cubes=0,
                max_num_aa_cubes=0,
    ...
    def _generate_objects(self, outFolder, uuid_str='', candShapes=[0,1,2], smooth_probability=1.0, no_hf=False, 
                          bPermuteMat=True, bScaleMesh=True, bMaxDimRange=[0.3, 0.5], repeat_times=1, idx=0):
        """
        ...
        for cube_idx, cube in enumerate(self.cubes + self.cubes_frame):
            ...
            # self.cubes[cube_idx] = (self.cubes[cube_idx][0], np.max(np.abs(ms.points), axis=0).tolist(), self.cubes[cube_idx][2])
        ...
        for _ in range(repeat_times):
            subFolder = Path(outFolder) / str(idx)
            ...
            max_dim, material_ids = self.genObj(subFolder + "/object.obj", outFolder, bMat=True, bComputeNormal=True, bScaleMesh=bScaleMesh, bMaxDimRange=bMaxDimRange)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_id', default=0, type=int, help='number of shapes to create')
    parser.add_argument('--end_id', default=1, type=int, help='number of shapes to create')
    ...
    parser.add_argument('--group_idx', default=0, type=int, help='number of times to repeat the generation of the same scene')
    args = parser.parse_args()
    seed_everything(args.seed)
    NUM = (args.group_idx % 3) * 2 + 3 # 3 5 7
    GROUP = (args.group_idx // 3)
    CUBE_TO_SHAPE_GROUPS={
        'lB': [
            {"sub_obj_nums": [1, 2, 3], "sub_obj_num_poss": [2, 3, 3]},
            {"sub_obj_nums": [4, 5, 6], "sub_obj_num_poss": [2, 3, 3]},
            {"sub_obj_nums": [7, 8, 9], "sub_obj_num_poss": [2, 3, 3]},
        ],
        'sB': [
            {"sub_obj_nums": [1, 2], "sub_obj_num_poss": [2, 3]},
            {"sub_obj_nums": [3, 4], "sub_obj_num_poss": [2, 3]},
            {"sub_obj_nums": [5, 6], "sub_obj_num_poss": [2, 3]},
        ]
    }
    CUBE_TO_SHAPE['lB'] = CUBE_TO_SHAPE_GROUPS['lB'][GROUP]
    CUBE_TO_SHAPE['sB'] = CUBE_TO_SHAPE_GROUPS['sB'][GROUP]
    
    project_dir = args.project_dir
    out_dir = f'{project_dir}/scenes'
    num_scenes = args.end_id - args.start_id
    dataset = 'rgb2x'
    uuid_str = ''
    ...
    for i in range(args.start_id, args.end_id):
        scene = Scene(max_num_large_cubes=NUM, max_num_small_cubes=NUM)
        new_uuid = i
