...
from common import *
from create_shapes_texture import *
...
class Scene(Shape):
    ...
    def _generate_objects(self, outFolder, uuid_str='', candShapes=[0,1,2], smooth_probability=1.0, no_hf=False, 
                          bPermuteMat=True, bScaleMesh=True, bMaxDimRange=[0.3, 0.5], repeat_times=1, idx=0, group_id=0):
        """
        ...
        for _ in range(repeat_times):
            ...
            max_dim, material_ids = self.genObj(subFolder + "/object.obj", outFolder, bMat=True, bComputeNormal=True, bScaleMesh=bScaleMesh, bMaxDimRange=bMaxDimRange, group_id=group_id)
...
if __name__ == '__main__':
    ...
    for i in range(args.start_id, args.end_id):
        new_uuid = i
        if os.path.exists(os.path.join(out_dir, str(new_uuid), "scenes.glb")):
            print(os.path.join(out_dir, str(new_uuid), "scenes.glb"), "exist!")
            continue
        else:
            os.system(f"rm -rf {os.path.join(out_dir, str(new_uuid))}")
            print(f"rm -rf {os.path.join(out_dir, str(new_uuid))}")
        scene = Scene()
