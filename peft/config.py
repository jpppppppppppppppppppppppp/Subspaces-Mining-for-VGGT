...
class LoraConfig(PeftConfig):
    ...
    Args:
        ...
        apply_svd_lora (`bool`):
            In this formulation, the LoRA update is rewritten as `A M B`, where `A` and `B` are fixed projection matrices and only `M` is
            trainable. When this flag is enabled, the fixed subspaces are constructed with the SVD-based strategy
            implemented in this repository.
        pattern_svd_lora (`str`):
            Controls where the parameterization is applied. Supported patterns are: `block`, which
            applies it to the whole transformer block; `attn`, which applies it only to attention layers; and `qkv`,
            which restricts it to the query/key/value projection layers.
        use_predefined_space (`str`):
            Optional path to a `.safetensors` file that stores predefined fixed matrices `A` and `B`. When provided,
            these matrices are loaded from the file instead of being constructed on the fly.
    ...
    apply_svd_lora: bool = field(
        default=False,
        metadata={
            "help": (
                "In this formulation, the LoRA update is rewritten as "
                "`A M B`, where `A` and `B` are fixed and only `M` is trainable. When enabled, the fixed subspaces "
                "are built with the SVD-based strategy."
            )
        },
    )
    pattern_svd_lora: str = field(
        default="block",
        metadata={
            "help": (
                "Scope for applying the parameterization. Use `block` to apply it to the whole "
                "transformer block, `attn` to apply it only to attention layers, or `qkv` to apply it only to the "
                "query/key/value projection layers."
            )
        },
    )
    use_predefined_space: str = field(
        default="",
        metadata={
            "help": (
                "Optional path to a `.safetensors` file containing predefined fixed matrices `A` and `B` for the "
                "parameterization. When set, these matrices are loaded from the file instead of being initialized "
