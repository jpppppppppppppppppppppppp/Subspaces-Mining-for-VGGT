...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        svd_pattern_flag = False
        current_key = self.kwargs.get("current_key", "")
        svd_pattern = self.kwargs.get("pattern_svd_lora", "")
        if svd_pattern == "qkv" and ".attn.qkv" in current_key and ((".frame_blocks." in current_key) or (".global_blocks." in current_key) or current_key.startswith("decoder.")):
            svd_pattern_flag = True
        if svd_pattern == "attn" and ".attn." in current_key and ((".frame_blocks." in current_key) or (".global_blocks." in current_key) or current_key.startswith("decoder.")):
            svd_pattern_flag = True
        if svd_pattern == "block" and ((".frame_blocks." in current_key) or (".global_blocks." in current_key) or current_key.startswith("decoder.")):
            svd_pattern_flag = True

        use_predefined_space_path = self.kwargs.get("use_predefined_space", "")
        use_predefined_space = False

        if use_predefined_space_path != "" and os.path.exists(use_predefined_space_path):
            split_current_key = current_key.split(".")
            if len(split_current_key) == 5 and split_current_key[0] == "aggregator":
                from safetensors.torch import load_file
                predefined_space = load_file(use_predefined_space_path)
                _, block_type, layer, module, weight = split_current_key
                if f"{block_type}.{module}.{weight}.{layer}.A" in predefined_space:
                    use_predefined_space = True
                    use_predefined_space_A = predefined_space[f"{block_type}.{module}.{weight}.{layer}.A"]
                    use_predefined_space_B = predefined_space[f"{block_type}.{module}.{weight}.{layer}.B"]
            elif len(split_current_key) == 4 and split_current_key[0] == "decoder":
                from safetensors.torch import load_file
                predefined_space = load_file(use_predefined_space_path)
                _, layer, module, weight = split_current_key
                if f"decoder.{module}.{weight}.{layer}.A" in predefined_space:
                    use_predefined_space = True
                    use_predefined_space_A = predefined_space[f"decoder.{module}.{weight}.{layer}.A"]
                    use_predefined_space_B = predefined_space[f"decoder.{module}.{weight}.{layer}.B"]
        elif use_predefined_space_path != "":
            raise ValueError(f"Predefined space path {use_predefined_space_path} does not exist.")

        if isinstance(self.base_layer, nn.Linear) and self.kwargs.get("apply_svd_lora", False) and svd_pattern_flag:
            self.is_linear_layer = True
            approx_rank = r
            approx_target = self.base_layer.weight.detach().cpu().numpy()
            u, s, vh = np.linalg.svd(approx_target)
            lora_AA = nn.Linear(self.in_features, approx_rank, bias=False)
            lora_AA.weight.data = torch.from_numpy(vh[:approx_rank, :]).contiguous()
            lora_AA.requires_grad_(False)
            lora_BB = nn.Linear(approx_rank, self.out_features, bias=False)
            lora_BB.weight.data = torch.from_numpy(u[:, :approx_rank]).contiguous()
            lora_BB.requires_grad_(False)
            self.lora_A[adapter_name] = nn.Sequential(lora_AA)
            self.lora_B[adapter_name] = nn.Sequential(nn.Linear(approx_rank, approx_rank, bias=False), lora_BB)
            self.lora_bias[adapter_name] = lora_bias
        elif isinstance(self.base_layer, nn.Linear) and use_predefined_space:
            self.is_linear_layer = True
            approx_rank = use_predefined_space_A.shape[0]
            lora_AA = nn.Linear(self.in_features, approx_rank, bias=False)
            lora_AA.weight.data = use_predefined_space_A
            lora_AA.requires_grad_(False)
            lora_BB = nn.Linear(approx_rank, self.out_features, bias=False)
            lora_BB.weight.data = use_predefined_space_B
            lora_BB.requires_grad_(False)
            self.lora_A[adapter_name] = nn.Sequential(lora_AA)
            self.lora_B[adapter_name] = nn.Sequential(nn.Linear(approx_rank, approx_rank, bias=False), lora_BB)
            self.lora_bias[adapter_name] = lora_bias
        ...
        if not ((".frame_blocks." in current_key) or (".global_blocks." in current_key)):
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            if init_lora_weights is True:
                ...
                if isinstance(self.lora_A[adapter_name], nn.Sequential):
                    if self.is_linear_layer:
                        pass
                    else:
                        nn.init.kaiming_uniform_(self.lora_A[adapter_name][1].weight, a=math.sqrt(5))
                else:
            ...
            if isinstance(self.lora_B[adapter_name], nn.Sequential):
                nn.init.zeros_(self.lora_B[adapter_name][0].weight)
            else:
                nn.init.zeros_(self.lora_B[adapter_name].weight)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                dropout = self.lora_dropout[active_adapter]
                scaling = self.scaling[active_adapter]
                if isinstance(lora_A, nn.Sequential):
                    x = self._cast_input_dtype(x, lora_A[0].weight.dtype)
                else:
...
def dispatch_default(
    ...
    **kwargs,
    ...
    if parameter_name is not None:
        ...
        # print(current_key)
        # if ".attn.qkv" in current_key:
        # if ".attn." in current_key:
        # if (".frame_blocks." in current_key) or (".global_blocks." in current_key):
            # kwargs["is_target_attn_layer"] = True
