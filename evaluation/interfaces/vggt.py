...
def infer_mv_pointclouds(filelist: List[str], model: VGGT, hydra_cfg: DictConfig, data_size: Tuple[int, int]):
    ...
    return point_map_by_unprojection, depth_map

def infer_mesh_pointclouds(filelist: List[str], model: VGGT, hydra_cfg: DictConfig, data_size: Tuple[int, int]):
    # bfloat16 is supported on Ampere GPUs (Compute Capability 8.0+) 
    dtype = torch.bfloat16 if torch.cuda.get_device_capability()[0] >= 8 else torch.float16
    ...
    with torch.no_grad():
        with torch.amp.autocast(hydra_cfg.device, dtype=dtype):
            # Predict attributes including cameras, depth maps, and point maps.
            predictions = model(images)
    pred_points_world = predictions["world_points"].squeeze(0).cpu().numpy()  # (N, H, W, 3)

    return pred_points_world