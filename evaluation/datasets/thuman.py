...
def read_depth(path: str, scale_adjustment=1.0) -> np.ndarray:
    if path.lower().endswith(".exr"):
        ...
        d = cv2.imread(path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH).squeeze()
        d[d > 1e9] = 0.0
    else:
...
class THuman(Dataset):
    def __init__(
        ...
        cache_file: str = "data/dataset_cache/THuman_mv_recon_cache.npy",
    ):

        self.THuman_DIR = THuman_DIR
        self.model_DIR = model_DIR
        if THuman_DIR == None:
            raise NotImplementedError
        print(f"THuman_DIR is {THuman_DIR}")
        category = sorted(os.listdir(THuman_DIR))
        self.split = split
        assert split == 'test', "Only test set preprocessed."
        if self.split == 'train':
            seq_numbers = category[:len(category)-100]
        ...
        if osp.exists(cache_file):
            print(f"[THuman] Loading from cache file: {cache_file}")
            self.metadata = np.load(cache_file, allow_pickle=True).item()
            self.sequence_list = sorted(list(self.metadata.keys()))
        else:
            print(f"[THuman] Cache file not found, loading from {THuman_DIR}")
            self.sequence_list = seq_numbers

            self.metadata = {}
            for seq in tqdm(self.sequence_list):
                rgb_root = osp.join(THuman_DIR, seq, 'renderings')
        ...
        print(f"[THuman] Data size: {len(self)}")
    ...
    def get_data(
        ...
        image_path = osp.join(self.THuman_DIR, seq_name, "renderings")
        depth_path = osp.join(self.THuman_DIR, seq_name, "renderings")
        cam_path = osp.join(self.THuman_DIR, seq_name, "opencv_cameras.json")
        ...
        for id_index, id in enumerate(ids):
            impath = osp.join(image_path, f"{id:03d}_rgba.png")
            depthpath = osp.join(depth_path, f"{id:03d}_depth.exr")
            ...
            norm = np.linalg.norm(extrinsic, axis=0, ord=2)
            fac = np.mean(norm[:3])
            extrinsic[:3, :] /= fac
            ...
            images[id_index][:, ~torch.tensor(mask)] = 1.0
        ...
        if self.model_DIR is not None:
            if os.path.exists(osp.join(self.model_DIR, seq_name, "mesh_smplx.obj")):
                batch['mesh_path']   = osp.join(self.model_DIR, seq_name, "mesh_smplx.obj")
            else:
                batch['mesh_path']   = osp.join(self.model_DIR, seq_name, f"{seq_name}.ply")
