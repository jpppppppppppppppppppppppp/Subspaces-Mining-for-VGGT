...
def absolute_value_scaling2(
    ...
    for i in range(max_iters):
        ...
        predicted_aligned = s[:, None, None] * ground_truth_depth + t[:, None, None]
        if mask is not None:
            predicted_aligned = predicted_aligned * mask
            ground_truth_depth = ground_truth_depth * mask
        # Compute absolute error
        abs_error = torch.abs(predicted_aligned - predicted_depth)
...
def depth_evaluation(
    ...
    if max_depth is not None:
        ...
        maximum = torch.max(ground_truth_depth_original)
        mask = (ground_truth_depth_original > 0)
    ...
    # for i in range(predicted_depth_original.shape[0]):
    #     predicted_depth_vis = (predicted_depth[i]*mask[i]).cpu().numpy()
    #     ground_truth_depth_vis = (ground_truth_depth[i]*mask[i]).cpu().numpy()
    #     predicted_depth_vis = (predicted_depth_vis - np.min(predicted_depth_vis)) / (np.max(predicted_depth_vis) - np.min(predicted_depth_vis) + 1e-8) * 255.0
    #     ground_truth_depth_vis = (ground_truth_depth_vis - np.min(ground_truth_depth_vis)) / (np.max(ground_truth_depth_vis) - np.min(ground_truth_depth_vis) + 1e-8) * 255.0
    #     cv2.imwrite(os.path.join(debug_dir, f"{i:03d}_pred.png"), predicted_depth_vis.astype(np.uint8))
    #     cv2.imwrite(os.path.join(debug_dir, f"{i:03d}_gt.png"), ground_truth_depth_vis.astype(np.uint8))
    # ### debug end
    ...
    if metric_scale:
        ...
        batch = predicted_depth.shape[0]
        s_init = (
            torch.median(predicted_depth.reshape(batch, -1), dim=1)[0] / torch.median(ground_truth_depth.reshape(batch, -1), dim=1)[0]
        ).tolist()
        ...
        print(s, t)
        ground_truth_depth = s[:, None, None] * ground_truth_depth + t[:, None, None]
